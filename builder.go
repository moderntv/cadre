...
func (b *Builder) Build() (c *cadre, err error) {
	...
	for addr, httpServer := range httpServers {
		...
		if multiplexed {
			...
			// buildGrpc binds the address whenever it was set explicitly, so multiplexing that was triggered
			// by a shared address rather than by WithGRPCMultiplex leaves a listener holding the port. The
			// http server could not bind it afterwards.
			if c.grpcListener != nil {
				err = c.grpcListener.Close()
				if err != nil {
					err = fmt.Errorf("cannot release the standalone grpc listener on `%s`: %w", addr, err)
					return
				}
			}
...
func (b *Builder) buildHTTP(
	...
	for _, newServer := range b.httpOptions {
		...
		if existingServer, ok := mergedHTTPOptions[addr]; ok {
			b.logger.Debug().
				Str("addr", addr).
				Str("new_server", newServer.serverName).
				Str("existing_server", existingServer.serverName).
				Msg("merging http servers - they share a listening address")
